...
После запуска будет создана директория `<bin_name>_fuzz_dir`, внутри которой будут созданы папки `input` и `output`.
Будет запущена сессия `tmux` с двумя вкладками - на каждой будет инстанс фаззера `AFL++`.
