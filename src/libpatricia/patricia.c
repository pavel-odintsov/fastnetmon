...
int comp_with_mask(void* addr, void* dest, u_int mask) {
    if (/* mask/8 == 0 || */ memcmp(addr, dest, mask / 8) == 0) {
        ...
        if (mask % 8 == 0 || (((u_char*)addr)[n] & m) == (((u_char*)dest)[n] & m)) return (1);
    }
    return (0);
}
...
prefix_t* ascii2prefix(int family, char* string) {
    ...
    if ((cp = strchr(string, '/')) != NULL) {
        ...
        save[cp - string] = '\0';
        string            = save;
        if (bitlen < 0 || bitlen > maxbitlen) bitlen = maxbitlen;
    } else {
        bitlen = maxbitlen;
    }

    if (family == AF_INET) {
        if ((result = my_inet_pton(AF_INET, string, &sin)) <= 0) return (NULL);
...
#else
        if ((result = inet_pton(AF_INET6, string, &sin6)) <= 0) return (NULL);
#endif /* NT */
        return (New_Prefix(AF_INET6, &sin6, bitlen));
    }
#endif /* HAVE_IPV6 */
    ...
    if (patricia->head) {
        ...
        while (Xrn) {
            ...
            if (Xrn->prefix) {
                ...
                if (Xrn->data && func) func(Xrn->data);
            } else {
...
#endif /* PATRICIA_DEBUG */
    if (node->bit > bitlen || node->prefix == NULL) return (NULL);
...
void patricia_remove(patricia_tree_t* patricia, patricia_node_t* node) {
    ...
    if (node->r && node->l) {
        ...
        if (node->prefix != NULL) Deref_Prefix(node->prefix);
        node->prefix = NULL;
...
void lookup_then_remove(patricia_tree_t* tree, char* string) {
    ...
    if ((node = try_search_exact(tree, string))) patricia_remove(tree, node);
}
